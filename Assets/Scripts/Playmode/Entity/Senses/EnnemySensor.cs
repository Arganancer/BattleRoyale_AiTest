...
namespace Playmode.Entity.Senses
{
	...
	public class EnnemySensor : MonoBehaviour
	{
		...
		public void RemoveNullEnnemies()
		{
			ICollection<EnnemyController> ennemiesInSightToRemove = new List<EnnemyController>();
			foreach (var ennemy in ennemiesInSight)
			{
				if (ennemy == null)
				{
					ennemiesInSightToRemove.Add(ennemy);
				}
			}

			foreach (var ennemy in ennemiesInSightToRemove)
			{
				ennemiesInSight.Remove(ennemy);
			}
		}
